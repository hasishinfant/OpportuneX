...
export function InfiniteScrollList<T>({
  ...
  const defaultEmptyComponent = (
    <div className="text-center py-12 px-4">
      ...
      <p className="text-secondary-500">Try adjusting your search criteria or filters.</p>
    </div>
  );
      ...
      {showItemCount && (totalCount !== undefined || data.length > 0) && (
        <div className="mb-4 text-sm text-secondary-600">
          {totalCount !== undefined ? (
            <span>Showing {data.length} of {totalCount.toLocaleString()} {itemCountLabel}</span>
          ) : (
            <span>{data.length} {itemCountLabel} loaded</span>
          )}
        </div>
      )}
