...
export function VoiceSearchInterface({
  ...
  if (!isSupported) {
    return (
      <div className={cn('text-center p-4 bg-secondary-50 rounded-lg', className)}>
    ...
    <div className={cn('space-y-4', className)}>
        ...
        <div className="text-center">
          <div className="inline-flex items-center space-x-2 px-4 py-2 bg-blue-50 rounded-full">
            ...
            <span className="text-sm font-medium text-blue-700">
              {isRecording ? `Recording... ${formatTime(recordingTime)}` : 'Processing...'}
            </span>
          </div>
        </div>
      )}
      ...
      {!isRecording && !isProcessing && (
        <div className="text-center text-sm text-secondary-500 space-y-1">
          ...
          <p className="text-xs">
            Example: "Find AI hackathons in Mumbai" or "Show me remote internships"
          </p>
        </div>
      )}
    </div>
