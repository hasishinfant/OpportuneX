...
describe('Property Test: Opportunity Display Completeness', () => {
  ...
  const completeOpportunityGenerator = fc.record({
    ...
    requiredSkills: fc.array(fc.string({ minLength: 2, maxLength: 30 }), { minLength: 1, maxLength: 15 }),
    ...
    eligibilityCriteria: fc.array(fc.string({ minLength: 5, maxLength: 50 }), { minLength: 1, maxLength: 10 }),
    ...
    prizes: fc.array(fc.string({ minLength: 3, maxLength: 50 }), { maxLength: 10 }),
    applicationDeadline: fc.date({ min: new Date(), max: new Date(Date.now() + 365 * 24 * 60 * 60 * 1000) }),
    startDate: fc.option(fc.date({ min: new Date(), max: new Date(Date.now() + 400 * 24 * 60 * 60 * 1000) })),
    endDate: fc.option(fc.date({ min: new Date(), max: new Date(Date.now() + 450 * 24 * 60 * 60 * 1000) })),
    externalUrl: fc.webUrl(),
    sourceId: fc.string({ minLength: 3, maxLength: 20 }),
    tags: fc.array(fc.string({ minLength: 2, maxLength: 20 }), { minLength: 1, maxLength: 12 }),
        ...
        async (mockOpportunities: any[]) => {
          ...
          if (result.data && result.data.opportunities.length > 0) {
            result.data.opportunities.forEach(opportunity => {
              ...
              expect(opportunity.organizer.name.trim().length).toBeGreaterThan(2);
              expect(opportunity.organizer.type).toMatch(/^(corporate|startup|government|academic)$/);
              ...
              expect(Array.isArray(opportunity.requirements.eligibility)).toBe(true);
              expect(opportunity.requirements.eligibility.length).toBeGreaterThan(0);
              ...
              expect(opportunity.details.mode).toMatch(/^(online|offline|hybrid)$/);
    ...
    await fc.assert(
      fc.asyncProperty(
        fc.array(fc.record({
          ...
          organizerName: fc.option(fc.string({ minLength: 1, maxLength: 50 })),
          organizerType: fc.option(fc.constantFrom('corporate', 'startup', 'government', 'academic')),
          requiredSkills: fc.option(fc.array(fc.string({ minLength: 1, maxLength: 20 }), { maxLength: 5 })),
        ...
        async (incompleteOpportunities: any[]) => {
          ...
          const actuallyIncomplete = incompleteOpportunities.filter(opp => 
            !opp.id || !opp.title || !opp.organizerName || !opp.organizerType ||
            !opp.requiredSkills || !opp.applicationDeadline || !opp.type ||
            !opp.mode || !opp.externalUrl || opp.isActive === false ||
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (mockOpp: any) => {
          ...
          const result = await searchService.searchOpportunities({ query: 'test' });
          ...
          if (result.data && result.data.opportunities.length > 0) {
            ...
            expect(Array.isArray(displayedOpp.requirements.eligibility)).toBe(true);
            ...
            expect(displayedOpp.organizer.name.trim()).toBe(displayedOpp.organizer.name);
            expect(displayedOpp.description.trim()).toBe(displayedOpp.description);

            // URLs should be valid format
            expect(displayedOpp.externalUrl).toMatch(/^https?:\/\/.+/);

            // Enums should have valid values
            expect(['hackathon', 'internship', 'workshop']).toContain(displayedOpp.type);
            expect(['corporate', 'startup', 'government', 'academic']).toContain(displayedOpp.organizer.type);
            expect(['online', 'offline', 'hybrid']).toContain(displayedOpp.details.mode);
          }
        }
      ),
  ...
  it('should handle edge cases in opportunity data gracefully', async () => {
    ...
    for (const edgeOpp of edgeCaseOpportunities) {
      ...
      const result = await searchService.searchOpportunities({ query: 'edge case' });
