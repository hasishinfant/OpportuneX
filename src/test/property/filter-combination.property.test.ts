...
describe('Property Test: Filter Combination Correctness', () => {
  ...
  const opportunityGenerator = (filters: SearchFilters) =>
    fc.record({
      id: fc.string({ minLength: 5, maxLength: 20 }),
      title: fc.string({ minLength: 10, maxLength: 100 }),
      description: fc.string({ minLength: 20, maxLength: 300 }),
      type: filters.type
        ? fc.constant(filters.type)
      ...
      organizerType: filters.organizerType
        ? fc.constant(filters.organizerType)
        : fc.constantFrom('corporate', 'startup', 'government', 'academic'),
      requiredSkills: filters.skills
        ? fc.array(
            fc.oneof(
              fc.constantFrom(...filters.skills),
      ...
      mode: filters.mode
        ? fc.constant(filters.mode)
        : fc.constantFrom('online', 'offline', 'hybrid'),
      location: filters.location
        ? fc.constant(filters.location)
        : fc.option(fc.string({ minLength: 3, maxLength: 30 })),
  ...
  it('should return only opportunities that match ALL applied filters', async () => {
    await fc.assert(
      fc.asyncProperty(filtersGenerator, async (filters: SearchFilters) => {
        ...
        const matchingOpportunities = await fc.sample(
          opportunityGenerator(filters),
          10
        );
        ...
        if (result.data && result.data.opportunities.length > 0) {
          result.data.opportunities.forEach(opportunity => {
            ...
            if (filters.location) {
              ...
              expect(opportunity.details.location?.toLowerCase()).toContain(
    ...
    await fc.assert(
      fc.asyncProperty(
        fc.record({
          ...
          skills: fc.array(
            fc.constantFrom('JavaScript', 'Python', 'React', 'AI'),
            { minLength: 1, maxLength: 3 }
          ),
        }),
        async (complexFilters: Required<Omit<SearchFilters, 'location'>>) => {
          // Generate opportunities that match all filters
          const matchingOpps = await fc.sample(
            opportunityGenerator(complexFilters),
            5
          );
